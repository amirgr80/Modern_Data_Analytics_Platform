...
def build_transactional_obt(
    ...
    obt = priced.select(
        ...
        F.coalesce(F.col("c.category_id"), F.lit("Unknown")).alias("category_id"),
        F.coalesce(F.col("c.category_name"), F.lit("Unknown")).alias("category_name"),
        F.col("o.user_id").alias("user_id"),
        F.coalesce(F.col("price.price_history_id"), F.lit("Unknown")).alias("price_history_id"),
        ...
        F.coalesce(F.col("oi.quantity"), F.lit(0)).cast("int").alias("quantity"),
        F.coalesce(F.col("oi.unit_price"), F.lit(0)).cast("decimal(29,2)").alias("unit_price"),
        F.coalesce(F.col("oi.discount_amount"), F.lit(0)).cast("decimal(29,2)").alias("discount_amount"),
        F.coalesce(F.col("oi.item_total_amount"), F.lit(0)).cast("decimal(29,2)").alias("item_total_amount"),
        F.coalesce(F.col("oi.price_difference"), F.lit(0)).cast("decimal(29,2)").alias("price_difference"),
        F.coalesce(F.col("o.total_amount"), F.lit(0)).cast("decimal(29,2)").alias("order_total_amount"),
